...
class AgentActorManager:
    ...
    async def run_llm_loop_async(self, gen_batch: DataProto, **sampling_params: Dict[str, Any]) -> Tuple[Dict, Dict]:
        ...
        print("\n" + "="*60)
        print("DEBUG: 检查batch中的数据")
        print("="*60)
        
        if hasattr(gen_batch, 'non_tensor_batch'):
            keys = list(gen_batch.non_tensor_batch.keys())
            print(f"找到的数据字段: {keys}")
            
            # 看看有没有extra_info
            if 'extra_info' in gen_batch.non_tensor_batch:
                extra = gen_batch.non_tensor_batch['extra_info']
                if len(extra) > 0:
                    print(f"✓ extra_info第一条: {extra[0]}")
                    if isinstance(extra[0], dict) and 'images' in extra[0]:
                        print(f"✓ 图片路径: {extra[0]['images']}")
                        print(f"✓ task_id: {extra[0].get('qid', 'not found')}")
        print("="*60 + "\n")
