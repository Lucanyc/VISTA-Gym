...
class TabMWPTask(VisionQATask):
    ...
    def __init__(self, task_id: str, adapter):
        """
        初始化TabMWP任务
        ...
        # 调用父类构造函数
        super().__init__(task_data, adapter)
    ...
    def _detect_table_type(self) -> str:
        ...
        # 安全地转换为小写
        table_text = table_text.lower() if table_text else ""
        table_title = table_title.lower() if table_title else ""
    ...
    def _requires_multi_step(self) -> bool:
        ...
        # 检查解题步骤说明
        solution = self.metadata.get("solution_explanation", "") if self.metadata else ""
        if solution:
            # 如果解题步骤包含多个步骤标识
    ...
    def check_success(self, action: Any) -> Tuple[bool, str]:
        ...
        if not success and self.answer:
            ...
            # 优先检查分数答案（在数值检查之前）
            if self._is_fraction_answer(correct_answer) or self._is_fraction_answer(action):
    ...
    def _is_fraction_answer(self, answer: str) -> bool:
        """判断是否为分数答案"""
        return "/" in answer and re.search(r'\d+\s*/\s*\d+', answer) is not None
    ...
    def _check_fraction_answer(self, user_answer: str, correct_answer: str) -> Tuple[bool, str]:
        ...
        try:
            ...
            if user_fraction and correct_fraction:
                # 直接比较Fraction对象（自动处理化简）
                if user_fraction == correct_fraction:
                    return True, "Correct fraction!"
                
                # 如果Fraction对象不相等，检查数值是否相等
                if abs(float(user_fraction) - float(correct_fraction)) < 0.001:
                    return True, "Correct fraction (equivalent)!"
            
            # 如果没有提取到分数，检查是否是小数形式的正确答案
            if correct_fraction and not user_fraction:
                user_decimal = self._extract_number(user_answer)
                if user_decimal is not None:
                    if abs(user_decimal - float(correct_fraction)) < 0.001:
                        return True, "Correct! (decimal equivalent of fraction)"
