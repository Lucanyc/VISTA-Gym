...
class VisionQAEnv:
    """视觉问答环境 - 支持复杂动作系统、DeepEyes工具、Grounding DINO和ChartMoE"""
    ...
    def step(self, action: str) -> Tuple[Dict[str, Any], float, bool, bool, Dict[str, Any]]:
        ...
        if action_result.get("type") in ["deepeyes_feedback", "tool_result"]:
            ...
            # 对Grounding DINO特殊处理
            # 在 step() 方法中，对Grounding DINO特殊处理部分
    ...
    def _execute_action(self, action: str) -> Dict[str, Any]:
        """执行动作 - 支持DeepEyes工具、Grounding DINO、ChartMoE、标准动作系统和直接答案"""
        if not action.strip():
            return {
                "type": "error",
                "status": "FAILED",
        ...
        if tool_type == "tool_call":
            ...
            print(f"  - diagram_formalizer_tool type: {type(self.diagram_formalizer_tool)}")
        ...
        if action_type == "tool_call":
            ...
            try:
                ...
                if done:
                    ...
                    # DeepEyes找到了答案（在action中包含<answer>标签）
                    import re
    ...
    def _get_obs(self) -> Dict[str, Any]:
        ...
        # 添加DeepEyes相关信息
        if self.enable_deepeyes_tools:
            obs["deepeyes_enabled"] = self.deepeyes_initialized
            obs["deepeyes_interactions"] = self.deepeyes_interaction_count
