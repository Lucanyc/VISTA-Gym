#这个是为所有工具和task设计的，这里建议最好分来，因为很多工具之间会不太协调
#已经分开：chartmoe，
#这个脚本是定义agent如何从env中使用tool的脚本
...
class VLMAgentWithTools(VLMAgent):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        print(f"[VLMAgentWithTools] Received config: {config}")
        print(f"[VLMAgentWithTools] max_new_tokens in config: {config.get('max_new_tokens', 'NOT FOUND')}")
    ...
    #def act(self, observation: Dict[str, Any]) -> Tuple[str, Dict[str, Any]]:
        ...
        if isinstance(observation, dict) and observation.get("must_use_tool"):
            ...
            if tool_to_use == "easyocr" and self.enable_easyocr:
    ...
    For multiple choice, just put the letter: <answer>A</answer>"""
                ...
                # 特殊处理：如果是EasyOCR反馈且问题需要文本内容
                if tool_feedback.get("tool") == "easyocr" and tool_feedback.get("success"):
                    print(f"  - EasyOCR feedback received, checking if answer needed")
                    
                    # 检查是否应该直接基于OCR结果回答
                    question_lower = observation.get("question", "").lower()
                    if any(kw in question_lower for kw in ["read", "what is written", "text says", "equation shown"]):
                        print(f"  - Direct answer from OCR results needed")
                        ...
                        clean_obs["output_format_instruction"] = f"""Based on the text extraction results:
    Detected texts: {', '.join(all_texts[:10])}{'...' if len(all_texts) > 10 else ''}
    ...
    def act(self, observation: Dict[str, Any]) -> Tuple[str, Dict[str, Any]]:
        ...
        # ⭐ 检查是否必须使用特定工具（用于强制 EasyOCR 和 ChartMoE）
        if isinstance(observation, dict) and observation.get("must_use_tool"):
            tool_to_use = observation.get("tool_to_use")
            tool_instruction = observation.get("tool_instruction", "")
            
            # 处理 ChartMoE 强制使用
            if tool_to_use == "chartmoe" and self.enable_chartmoe:
                if self.debug or True:  # Always show this important info
                    print(f"  - ⚠️ FORCED tool usage: {tool_to_use}")
                    print(f"  - Instruction: {tool_instruction[:100]}...")
...
<answer>{num_detections}</answer>"""
    ...
    def _get_available_tools(self, observation: Dict[str, Any]) -> List[int]:
        ...
        if "chartmoe" in available_tools_list:
            available.append(3)
        if "easyocr" in available_tools_list:  # 添加 EasyOCR
            available.append(4)
    ...
    def _handle_tool_feedback(self, observation: Dict[str, Any]) -> Tuple[str, Dict[str, Any]]:
        ...
        elif tool_name == "easyocr":  # 添加 EasyOCR
            num_detections = tool_feedback.get("num_detections", 0)
            task = tool_feedback.get("task", "unknown")
            response_parts.append(f"EasyOCR ({task}) detected {num_detections} text regions.")
