...
class VLMAgentWithTools(VLMAgent):
    ...
    Secondary support: DeepEyes, GroundingDINO, DiagramFormalizer, EasyOCR, SAM2, SymPy Geometry, MultiMath Server
    """
    ...
    def act(self, observation: Dict[str, Any]) -> Tuple[str, Dict[str, Any]]:
        ...
        # 几何任务特殊处理：优先使用 MultiMath Server（如果启用）
        if is_geometry:
            ...
            # 如果 MultiMath Server 可用，优先使用
            if 7 in available_tools and self._should_use_multimath_server(observation):
    ...
    def _get_available_tools(self, observation: Dict[str, Any]) -> List[int]:
        ...
        if observation.get("is_geometry_task", False):
            if self.enable_multimath_server and 7 not in available:
                available.append(7)
    ...
    def _analyze_tool_need(self, observation: Dict[str, Any]) -> bool:
        ...
        # 几何任务：如果有 MultiMath Server 或 SymPy 可用
