...
class VLMAgent(BaseAgent):
    ...
    def _generate_response(self, inputs: Dict[str, Any]) -> str:
        ...
        generation_config = {
            "max_new_tokens": agent_config.get("max_new_tokens", 2048),  # 从512增加到2048
            "temperature": agent_config.get("temperature", 0.1),
            "do_sample": agent_config.get("temperature", 0.1) > 0,
            "repetition_penalty": agent_config.get("repetition_penalty", 1.05),  # 从1.15降低到1.05
        }
        ...
        if inputs.get('_has_reflection', False):
            ...
            generation_config['max_new_tokens'] = max(original_max * 2, 3000)
            # 但也要有上限
