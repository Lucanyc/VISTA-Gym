#这个是专门为chartqa设计的agent使用tool的脚本
#不是最新的
...
class VLMAgentWithTools(VLMAgent):
    """
    VLM Agent with tool support - ChartQA optimized version
    ...
    def act(self, observation: Dict[str, Any]) -> Tuple[str, Dict[str, Any]]:
        ...
        # 清除已使用工具的must_use_tool标志
        if isinstance(observation, dict):
            # 如果ChartMoE已经使用过，清除强制标志
            if observation.get("chartmoe_history") and len(observation.get("chartmoe_history", [])) > 0:
            ...
            # 类似处理其他工具
            if observation.get("easyocr_history"):
                observation.pop("must_use_tool", None)
                observation.pop("tool_to_use", None)
        ...
        # ChartQA特殊处理：强制使用ChartMoE
        if is_chartqa and 3 in available_tools:  # 3是ChartMoE的索引
            # 检查是否已经使用过ChartMoE
            question_key = hashlib.md5(observation.get("question", "").encode()).hexdigest()
