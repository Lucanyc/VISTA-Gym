#这个最新的是专门为geometry3k设计的agent使用inter-gps的脚本
...
class VLMAgentWithTools(VLMAgent):
    ...
    Supports: DiagramFormalizer (geometry), GroundingDINO (object detection), ChartMoE (charts), DeepEyes (visual enhancement), EasyOCR (text extraction)
    Enhanced: Now supports Qwen-generated bbox for DeepEyes
    Modified: Added forced tool usage for Geometry3K tasks
    """
    ...
    TOOL_INDEX_MAP = {
        ...
        1: {
            "name": "inter_gps",
            "type": "geometry_logic_solver",
            "brief": "solve geometry problems using formal logic representations",
            "requires_params": True
        },
        2: {
            "name": "grounding_dino",
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        tool_config_keys = [
            ...
            "enable_easyocr",  # 添加 easyocr
            "grounding_dino_config",
            #"diagram_formalizer_config",
            "chartmoe_config",
            "deepeyes_config",  # 添加 deepeyes 配置
            "easyocr_config",  # 添加 easyocr 配置
            "inter_gps_config",  # 添加 inter_gps 配置
            "enable_inter_gps",  # 添加 inter_gps 启用配置
        ...
        self.enable_easyocr = tool_config.get("enable_easyocr", False)  # 添加 easyocr
        self.enable_inter_gps = tool_config.get("enable_inter_gps", False)  # 添加 inter_gps
        ...
        print(f"  - enable_easyocr: {self.enable_easyocr}")  # 添加打印
        #print(f"  - enable_diagram_formalizer: {self.enable_diagram_formalizer}")
        print(f" - enable_inter_gps: {self.enable_inter_gps}")  # 添加打印
        ...
        # 新增：定义工具链规则（添加 DeepEyes 和 EasyOCR 相关规则）
        self.tool_chain_rules = {
            ...
            "text_extraction": {  # 新增 EasyOCR 规则
                "description": "Problems that need text extraction from images",
                "primary": "easyocr",
                "secondary": "grounding_dino",
                "condition": r'\bread|text|written|equation|formula|label|sign|inscription|caption\b',
                "examples": ["read the text", "what is written", "equation in the image", "text on the sign"]
            },
            "math_formula": {  # 新增：数学公式提取
                "description": "Problems involving mathematical formulas or equations",
                "primary": "easyocr",
                "secondary": "inter_gps",   
                "condition": r'\bequation|formula|expression|solve.*written|written.*solve\b',
                "examples": ["solve the equation written", "what formula is shown", "mathematical expression"]
            },
            "geometry_counting": {
            ...
            "chart_text": {  # 新增：图表文本提取
                "description": "Chart problems that need text extraction",
                "primary": "chartmoe",
                "secondary": "easyocr",
                "condition": r'\blabel|title|axis.*name|legend|caption\b',
                "examples": ["What is the chart title", "Read the axis labels", "Legend text"]
            }
        }
        ...
        self.inter_gps_tool = None  # 添加 Inter-GPS 工具实例
    ...
    def act(self, observation: Dict[str, Any]) -> Tuple[str, Dict[str, Any]]:
        ...
        # 检测是否是Geometry3K任务
        is_geometry3k = (
            observation.get("dataset") == "geometry3k" or 
            (observation.get("diagram_formalizer_enabled", False) and
             self._is_geometry_problem(observation.get("question", "")))
        )
        
        if is_geometry3k:
            print(f"  - ✓ Geometry3K task detected")
        ...
        # 如果已经用过 EasyOCR，就忽略 must_use_tool
        if isinstance(observation, dict) and observation.get("easyocr_history"):
            observation.pop("must_use_tool", None)
            observation.pop("tool_to_use", None)
            observation.pop("tool_instruction", None)
            observation.pop("easyocr_forced_in_current_step", None)
            if True:  # Always show this info
                print(f"  - ✓ EasyOCR already used, cleared must_use_tool flags")
        
        # 如果已经用过 Inter-GPS，在非反思阶段清除强制标志
        if isinstance(observation, dict) and observation.get("inter_gps_history") and len(observation.get("inter_gps_history", [])) > 0 and not is_reflection:
            observation.pop("must_use_tool", None)
            observation.pop("tool_to_use", None)
            print(f"  - ✓ Inter-GPS already used, cleared must_use_tool flags")
        
        # 如果已经用过 ChartMoE，也忽略 must_use_tool
        if isinstance(observation, dict) and observation.get("chartmoe_history") and len(observation.get("chartmoe_history", [])) > 0:
            observation.pop("must_use_tool", None)
            observation.pop("tool_to_use", None)
            if True:  # Always show this info
                print(f"  - ✓ ChartMoE already used, cleared must_use_tool flags")
        
        # 如果已经用过 DiagramFormalizer，在非反思阶段清除强制标志
        if isinstance(observation, dict) and observation.get("diagram_formalizer_history") and len(observation.get("diagram_formalizer_history", [])) > 0 and not is_reflection:
            observation.pop("must_use_tool", None)
            observation.pop("tool_to_use", None)
            print(f"  - ✓ DiagramFormalizer already used, cleared must_use_tool flags")
        
        # ⭐ 检查是否必须使用特定工具（包括DiagramFormalizer）
        if isinstance(observation, dict) and observation.get("must_use_tool"):
            tool_to_use = observation.get("tool_to_use")
            tool_instruction = observation.get("tool_instruction", "")
            
            # 处理 DiagramFormalizer 强制使用
            if tool_to_use == "diagram_formalizer" and self.enable_diagram_formalizer:
                if self.debug or True:  # Always show this important info
                    print(f"  - ⚠️ FORCED tool usage: {tool_to_use}")
                    print(f"  - Instruction: {tool_instruction[:100]}...")
                
                # 直接生成 DiagramFormalizer 工具调用
                tool_call = self._build_diagram_formalizer_prompt(observation)
                
                # 更新工具调用计数
                self.current_tool_calls += 1
                self.tool_use_stats["total_calls"] += 1
                self.tool_use_stats["diagram_formalizer"] = self.tool_use_stats.get("diagram_formalizer", 0) + 1
                ...
                print(f"  - Generated forced DiagramFormalizer call")
                print(f"[VLMAgentWithTools.act] DEBUG END\n")
                return tool_call, extra_info
            
            # 处理 ChartMoE 强制使用
            elif tool_to_use == "chartmoe" and self.enable_chartmoe:
                if self.debug or True:  # Always show this important info
                    print(f"  - ⚠️ FORCED tool usage: {tool_to_use}")
                    print(f"  - Instruction: {tool_instruction[:100]}...")
                ...
                tool_call = {
                    "tool": "chartmoe",
                    "task": "to_table"  # 默认使用 to_table 任务
                }
                response = f'<tool_call>{json.dumps(tool_call)}</tool_call>'
                
                # 更新工具调用计数
                self.current_tool_calls += 1
                self.tool_use_stats["total_calls"] += 1
                self.tool_use_stats["chartmoe"] = self.tool_use_stats.get("chartmoe", 0) + 1
                ...
                print(f"  - Generated forced ChartMoE call")
                print(f"[VLMAgentWithTools.act] DEBUG END\n")
                return response, extra_info
            
            elif tool_to_use == "easyocr" and self.enable_easyocr:
                if self.debug or True:  # Always show this important info
                    print(f"  - ⚠️ FORCED tool usage: {tool_to_use}")
                    print(f"  - Instruction: {tool_instruction[:100]}...")
                ...
                self.current_tool_calls += 1
                self.tool_use_stats["total_calls"] += 1
                self.tool_use_stats["easyocr"] = self.tool_use_stats.get("easyocr", 0) + 1
                ...
                print(f"  - Generated forced EasyOCR call")
                print(f"[VLMAgentWithTools.act] DEBUG END\n")
                return response, extra_info
    ...
    For multiple choice, just put the letter: <answer>A</answer>"""
                ...
                # 特殊处理：如果是EasyOCR反馈
                if tool_feedback.get("tool") == "easyocr" and tool_feedback.get("success"):
                    print(f"  - EasyOCR feedback received, processing OCR results")
                    
                    # 对于所有EasyOCR反馈，都要处理并传递给模型
                    return self._handle_easyocr_feedback(observation)
                
                
                if tool_feedback.get("tool") == "inter_gps":
                    print(f"  - Inter-GPS feedback received")
                    
                    solution = tool_feedback.get("solution", "")
                    if solution:
                        # Inter-GPS 提供了明确答案，直接返回
                        print(f"  - Inter-GPS found solution: {solution}")
                        
                        response = f"""<think>
        Inter-GPS solved the geometry problem using formal logic.
        Solution: {solution}
        Method: {tool_feedback.get('method', 'Inter-GPS')}
        </think>
        <answer>{solution}</answer>"""
                        ...
                        extra_info = {
                            ...
                            "tool_used": "inter_gps",
                            "tool_solution": solution,
                            "answered_with_format": True,
                            "stage": "final_answer"
                        }
                        
                        return response, extra_info
                    else:
                        # 如果Inter-GPS没有找到解决方案，继续工具链
                        print(f"  - Inter-GPS did not find direct solution, continuing tool chain")
    ...
    For multiple choice, just put the letter: <answer>A</answer>"""
            ...
            # 特殊处理EasyOCR反馈（非协同模式）
            elif tool_feedback.get("tool") == "easyocr":
                return self._handle_easyocr_feedback(observation)
            
            # 特殊处理Inter-GPS反馈
            elif tool_feedback.get("tool") == "inter_gps":
                return self._handle_inter_gps_feedback(observation)
        ...
        # ⭐ 新增：Geometry3K特殊处理 - 首次强制使用Inter-GPS
        if is_geometry3k and 1 in available_tools and not is_reflection:
            question = observation.get("question", "")
            current_attempt = observation.get("attempt", 1)
            question_key = hashlib.md5(f"{question}_attempt_{current_attempt}".encode()).hexdigest()
            
            if question_key not in self.tool_call_history or "inter_gps" not in self.tool_call_history.get(question_key, set()):
                print(f"  - Geometry3K task: forcing Inter-GPS usage")
                tool_call = self._build_inter_gps_prompt(observation)
                
                # 记录工具调用
                if question_key not in self.tool_call_history:
                    self.tool_call_history[question_key] = set()
                self.tool_call_history[question_key].add("inter_gps")
                ...
                extra_info = {
                    ...
                    "tool_call_count": self.current_tool_calls,
                    "tool_used": "inter_gps",
                    "reason": "Geometry3K task requires formal logic analysis",
                    "first_call": True,
                    "log_tool_usage": True
                }
        ...
        # ⭐ 新增：强制 ChartQA 使用工具
        if observation.get("is_visual_question", False) and observation.get("chartmoe_enabled", False):
            if self._should_use_chartmoe(observation):
                tool_needed = True
                print(f"  - Forcing tool use for ChartQA visual question")
    ...
    def _get_planned_tools(self, observation: Dict[str, Any]) -> List[str]:
        ...
        if not planned:
            ...
            elif self._should_use_easyocr(observation):  # 添加 EasyOCR
                planned.append("easyocr")
    ...
    def decide_tool(self, observation: Dict[str, Any]) -> Optional[str]:
            ...
            if re.search(rule["condition"], question):
                ...
                if primary_tool not in tc:
                    ...
                    if primary_tool == "deepeyes":
                        return self._build_deepeyes_prompt(observation)
                    elif primary_tool == "inter_gps":
                        return self._build_inter_gps_prompt(observation)
                    ...
                    elif primary_tool == "easyocr":  # 添加 EasyOCR
                        return self._build_easyocr_prompt(observation)
                ...
                if secondary_tool not in tc:
                    ...
                    if self._should_use_secondary_tool(primary_tool, tc[primary_tool], question):
                        ...
                        if secondary_tool == "grounding_dino":
                            return self._build_grounding_dino_prompt(observation)
                        elif secondary_tool == "inter_gps": 
                            return self._build_inter_gps_prompt(observation)
                        elif secondary_tool == "chartmoe":
                            return self._build_chartmoe_prompt(observation)
                        elif secondary_tool == "deepeyes":
                            return self._build_deepeyes_prompt(observation)
                        elif secondary_tool == "easyocr":  # 添加 EasyOCR
                            return self._build_easyocr_prompt(observation)
        ...
        if not tc:  # 还没有使用任何工具
            ...
            # 文本提取优先（如果明确需要读取文本）
            if self._should_use_easyocr(observation) and "easyocr" not in tc:
                print(f"[decide_tool] Text extraction needed, using EasyOCR")
                return self._build_easyocr_prompt(observation)
            ...
            # 逻辑形式优先（如果有逻辑形式可用）
            if self._should_use_inter_gps(observation) and "inter_gps" not in tc:
                print(f"[decide_tool] Logic forms available, using Inter-GPS")
                return self._build_inter_gps_prompt(observation)
    ...
    def _should_use_inter_gps(self, observation: Dict[str, Any]) -> bool:
        """判断是否应该使用 Inter-GPS 工具"""
        # 检查是否有逻辑形式
        metadata = observation.get("metadata", {})
        has_logic_form = metadata.get("has_logic_form", False)
        
        if not has_logic_form:
            return False
        
        # 检查是否是几何问题
        task = observation.get("task", "")
        if task == "geometry_qa" or task == "geometry problem solving":
            return True
        
        # 检查问题类型
        question = observation.get("question", "").lower()
        if self._is_geometry_problem(question):
            # 如果有逻辑形式，优先使用 Inter-GPS
            return True
    ...
    def _should_use_easyocr(self, observation: Dict[str, Any]) -> bool:
        ...
        # 第一步：检查是否是纯计算问题
        if self._is_pure_calculation(question):
            print(f"  - Pure calculation detected, skipping EasyOCR")
            return False
        ...
        text_extraction_keywords = [
            "read", "text", "written", "write", "writing",
            "equation", "formula", "expression",
            "label", "sign", "inscription", "caption",
            "title", "name", "word", "letter",
            "says", "states", "shows text",
            "ocr", "recognize text", "extract text"
        ]
        
        # 数学相关的文本提取
        math_text_keywords = [
            "equation written", "formula shown", "expression written",
            "solve.*written", "written.*solve",
            "mathematical text", "math notation"
        ]
        
        # 检查是否包含文本提取关键词
        if any(keyword in question for keyword in text_extraction_keywords):
            print(f"  - Text extraction keywords detected in question")
            return True
        
        # 检查是否包含数学文本关键词
        if any(re.search(pattern, question) for pattern in math_text_keywords):
            print(f"  - Mathematical text extraction needed")
            return True
        
        # 特定任务类型检查
        task_type = observation.get("task_type", "").lower()
        if any(task in task_type for task in ["text recognition", "ocr", "reading"]):
            print(f"  - Task type suggests text extraction needed")
            return True
        
        # 检查是否需要提取表格文本
        if "table" in question and any(word in question for word in ["read", "text", "value", "data"]):
            print(f"  - Table text extraction needed")
            return True
    ...
    def _analyze_tool_need(self, observation: Dict[str, Any]) -> bool:
        ...
        # ⭐ 新增：Geometry3K任务强制需要工具
        if observation.get("diagram_formalizer_enabled", False) and self._is_geometry_problem(question):
            print(f"  - Geometry task with DiagramFormalizer enabled, tool needed")
            return True
        
        # ⭐ 新增：如果是视觉问题且启用了 ChartMoE，应该使用工具
        if observation.get("is_visual_question", False) and observation.get("chartmoe_enabled", False):
            if self._should_use_chartmoe(observation):
                print(f"  - Visual question with ChartMoE enabled, tool needed")
                return True
        ...
        # 文本提取问题 - 需要 EasyOCR
        if self._should_use_easyocr(observation):
            return True
    ...
    def _generate_tool_call(self, observation: Dict[str, Any], available_tools: List[int]) -> Tuple[str, Dict[str, Any]]:
        ...
        # 添加 EasyOCR 支持
        if self._should_use_easyocr(observation) and 4 in available_tools:
            tool_name = "easyocr"
            # 检查是否已调用过
        ...
        # 添加 DeepEyes 支持
        elif self._should_use_deepeyes(observation) and 0 in available_tools:
            tool_name = "deepeyes"
            # 检查是否已调用过
            if question_key in self.tool_call_history and tool_name in self.tool_call_history[question_key]:
                print(f"  - Tool {tool_name} already called for this question, skipping")
        ...
        elif self._should_use_inter_gps(observation) and 1 in available_tools:
            tool_name = "inter_gps"
            # 检查是否已调用过
            if question_key in self.tool_call_history and tool_name in self.tool_call_history[question_key]:
                print(f"  - Tool {tool_name} already called for this question, skipping")
...
<answer>{num_detections}</answer>"""
    ...
    def _get_available_tools(self, observation: Dict[str, Any]) -> List[int]:
        ...
        # 特殊处理：如果观察中明确启用了diagram_formalizer
        if observation.get("inter_gps_enabled", False) and 1 not in available:
            available.append(1)
            print(f"  - Inter-GPS enabled from observation")
        
        # 特殊处理：如果观察中明确启用了chartmoe
        if observation.get("chartmoe_enabled", False) and 3 not in available:
            available.append(3)
            print(f"  - ChartMoE enabled from observation")
        
        print(f"[DEBUG _get_available_tools] available tools indices: {available}")
        tool_names = [self.TOOL_INDEX_MAP[idx]["name"] for idx in available if idx in self.TOOL_INDEX_MAP]
    ...
    def _is_geometry_problem(self, question: str) -> bool:
        ...
        geometry_keywords = [
            ...
            'parallelogram', 'rhombus', 'trapezoid', 'quadrilateral',  # 添加更多几何形状
            'side', 'vertex', 'edge', 'diagonal', 'bisect',  # 几何元素
            'find', 'calculate', 'prove', 'given',  # 几何问题常见动词
            'length', 'width', 'height', 'base',  # 尺寸相关
            'theorem', 'formula', 'property'  # 几何理论相关
        ]
    ...
    def _should_use_chartmoe(self, observation: Dict[str, Any]) -> bool:
        """判断是否应该使用 ChartMoE - 增强版"""
        question = observation.get("question", "").lower()
        task = observation.get("task", "")
        
        # ⭐ 新增：如果明确标记为图表问题，直接返回 True
        if observation.get("is_visual_question", False) and observation.get("chartmoe_enabled", False):
            print(f"  - Visual question with ChartMoE enabled, should use ChartMoE")
            return True
        ...
        chart_keywords = [
            ...
            "increase", "decrease", "correlation", "compare",
            "highest", "lowest", "peak", "valley",
            "favorable", "unfavorable", "view", "percentage",  # 添加更多关键词
            "which line", "deduct", "difference between"
        ]
        ...
        if any(keyword in question for keyword in chart_keywords):
            print(f"  - Chart keywords detected in question")
            return True
        
        # 检查是否有图像且问题涉及数据比较
        has_image = observation.get("has_image", False)
        if has_image and any(kw in question for kw in ["compare", "which", "more", "less", "difference"]):
            print(f"  - Image with comparison question, likely a chart")
            return True
    ...
    def _build_inter_gps_prompt(self, observation: Dict[str, Any]) -> str:
        """构建 Inter-GPS 工具调用提示"""
        # 获取任务ID（如果有）
        task_id = observation.get("task_id", "")
        
        # 获取逻辑形式（如果在 observation 中）
        metadata = observation.get("metadata", {})
        ...
        # 如果有task_id，添加它
        if task_id:
            tool_call["parameters"]["problem_id"] = task_id
        
        # 如果有逻辑形式，直接传递
        if text_logic and diagram_logic:
            tool_call["parameters"]["text_logic"] = text_logic
            tool_call["parameters"]["diagram_logic"] = diagram_logic
    ...
    def _build_chartmoe_prompt(self, observation: Dict[str, Any]) -> str:
        """构建 ChartMoE 工具调用提示 - 修正版"""
        question = observation.get("question", "")
        
        # ⭐ 使用正确的格式
        tool_call = {
            "tool": "chartmoe",
            "task": "to_table"  # 默认先转换为表格
        }
        
        # 根据问题类型可以调整任务
        question_lower = question.lower()
        if any(kw in question_lower for kw in ["describe", "explain", "what shows"]):
            tool_call["task"] = "to_text"
        elif any(kw in question_lower for kw in ["analyze", "trend", "pattern"]):
            tool_call["task"] = "analyze"
        elif any(kw in question_lower for kw in ["compare", "difference", "which"]):
            tool_call["task"] = "compare"
    ...
    def _build_easyocr_prompt(self, observation: Dict[str, Any]) -> str:
        ...
        # 根据问题类型决定任务
        if any(kw in question_lower for kw in ["equation", "formula", "mathematical", "expression"]):
            task = "extract_math"
        elif "table" in question_lower:
            task = "extract_table"
        elif any(kw in question_lower for kw in ["specific", "region", "area", "part"]):
        ...
        # 如果是区域提取，尝试确定区域
        if task == "extract_by_region":
            # 简单的区域推断逻辑
            if "top" in question_lower:
                tool_call["parameters"]["region"] = [0, 0, 800, 200]  # 假设图像尺寸
            elif "bottom" in question_lower:
                tool_call["parameters"]["region"] = [0, 400, 800, 600]
            # 可以添加更多区域推断逻辑
    ...
    def _should_use_secondary_tool(self, primary_tool: str, primary_result: Dict, question: str) -> bool:
        """判断是否需要使用次要工具"""
        # EasyOCR 后是否需要其他工具
        if primary_tool == "easyocr":
            # 如果提取的是数学公式，可能需要 DiagramFormalizer
            if "math_elements" in primary_result and primary_result.get("math_elements", {}).get("expressions"):
                if re.search(r'\bsolve|calculate|compute\b', question.lower()):
                    return True
            # 如果问题涉及定位文本，可能需要 GroundingDINO
            if re.search(r'\bwhere|position|locate.*text\b', question.lower()):
                return True
        
        # DeepEyes 后是否需要 GroundingDINO
        elif primary_tool == "deepeyes":
    ...
    def _handle_tool_feedback(self, observation: Dict[str, Any]) -> Tuple[str, Dict[str, Any]]:
        ...
        # 更新统计
        if tool_name in ["deepeyes", "diagram_formalizer", "grounding_dino", "chartmoe", "easyocr"]:
            self.tool_use_stats["successful_calls"] += 1
        
        # 特殊处理不同的工具反馈
        if tool_name == "diagram_formalizer":
            return self._handle_diagram_formalizer_feedback(observation)
        elif tool_name == "grounding_dino":
            return self._handle_grounding_dino_feedback(observation)
        elif tool_name == "chartmoe":
            return self._handle_chartmoe_feedback(observation)
        elif tool_name == "deepeyes":
            return self._handle_deepeyes_feedback(observation)
        elif tool_name == "easyocr":
            return self._handle_easyocr_feedback(observation)
        elif tool_name == "inter_gps":
            if tool_feedback.get("solution"):
                response_parts.append(f"Inter-GPS solution found: {tool_feedback['solution']}")
            else:
                response_parts.append(f"Inter-GPS analysis completed (no direct solution)")
        # 默认处理
        return self._handle_generic_tool_feedback(observation)
    ...
    def _build_final_answer(self, observation: Dict[str, Any]) -> Tuple[str, Dict[str, Any]]:
        ...
        if "inter_gps" in self.tool_context:
            gps_data = self.tool_context["inter_gps"]
            solution = gps_data.get("solution", "")
            method = gps_data.get("method", "")
            
            gps_summary = ["=== Formal Logic Solver (Inter-GPS) ==="]
            if solution:
                gps_summary.append(f"Solution: {solution}")
                gps_summary.append(f"Method: {method}")
            
            if gps_data.get("equations"):
                gps_summary.append(f"Equations used: {len(gps_data['equations'])}")
            
            if gps_data.get("constraints"):
                gps_summary.append("Constraints:")
                for k, v in list(gps_data["constraints"].items())[:5]:
                    gps_summary.append(f"  {k} = {v}")
        ...
        # 5. 汇总 EasyOCR 信息（新增）
        if "easyocr" in self.tool_context:
            ocr_data = self.tool_context["easyocr"]
            task = ocr_data.get("task", "unknown")
            all_texts = ocr_data.get("all_texts", [])
            num_detections = ocr_data.get("num_detections", 0)
            
            ocr_summary = [f"=== Text Extraction (EasyOCR - {task}) ==="]
            ocr_summary.append(f"Detected {num_detections} text regions")
            
            if task == "extract_math" and "math_elements" in ocr_data:
                math_elements = ocr_data["math_elements"]
                ocr_summary.append("Mathematical elements found:")
                if math_elements.get("numbers"):
                    ocr_summary.append(f"  Numbers: {', '.join(e['value'] for e in math_elements['numbers'][:5])}")
                if math_elements.get("symbols"):
                    ocr_summary.append(f"  Symbols: {', '.join(e['value'] for e in math_elements['symbols'][:5])}")
                if math_elements.get("expressions"):
                    ocr_summary.append(f"  Expressions: {', '.join(e['value'] for e in math_elements['expressions'][:3])}")
            elif task == "extract_table" and "table_data" in ocr_data:
                table_data = ocr_data["table_data"]
                ocr_summary.append(f"Extracted table with {len(table_data)} rows")
            else:
                # 显示提取的文本
                if all_texts:
                    ocr_summary.append("Extracted texts:")
                    for i, text in enumerate(all_texts[:10]):
                        ocr_summary.append(f"  {i+1}. {text}")
                    if len(all_texts) > 10:
                        ocr_summary.append(f"  ... and {len(all_texts) - 10} more")
    ...
    - EasyOCR for text extraction and recognition
    ...
    def _handle_easyocr_feedback(self, observation: Dict[str, Any]) -> Tuple[str, Dict[str, Any]]:
        """处理EasyOCR反馈，确保OCR结果被传递给模型进行分析"""
        print(f"  - Processing EasyOCR feedback")
        
        tool_feedback = observation.get("tool_feedback", {})
        
        # 提取OCR结果
        all_texts = tool_feedback.get("all_texts", [])
        processed_output = tool_feedback.get("processed_output", "")
        num_detections = tool_feedback.get("num_detections", 0)
    ...
    ================================
    Number of text regions found: {num_detections}
    All detected texts: {all_texts}
    Combined text output: {processed_output}
    ================================

    Based on the OCR results above, I can see the following text elements in the image:"""
        
        for i, text in enumerate(all_texts, 1):
            ocr_details += f"\n{i}. {text}"
        
        # 获取原始的输出格式指令
        original_instruction = observation.get('output_format_instruction', '')
        
        # 根据题目类型调整输出格式指令
        if observation.get("subfield") in ["Geometry", "Algebra", "Number Theory", "Combinatorics"]:
            # 数学题目：强调使用提取的公式和数字
            enhanced_obs["output_format_instruction"] = f"""{ocr_details}

    IMPORTANT: Use the OCR-detected text above in your solution. The image contains: {', '.join(all_texts)}

    Now solve this {observation.get('subfield', 'mathematical')} problem using the detected information.

    {original_instruction}

    ⚠️ CRITICAL FORMAT REQUIREMENT:
    You MUST end your response with your answer in ONE of these formats:
    1. <answer>your_answer</answer>
    2. \\boxed{{your_answer}}

    Examples:
    - If answer is 1/2: <answer>1/2</answer> OR \\boxed{{1/2}}
    - If answer is x^2+1: <answer>x^2+1</answer> OR \\boxed{{x^2+1}}
    - If answer is (2,3): <answer>(2,3)</answer> OR \\boxed{{(2,3)}}

    FAILURE TO USE PROPER FORMAT WILL RESULT IN INCORRECT EVALUATION!"""
        
        elif observation.get("subfield") in ["Mechanics", "Electromagnetism", "Thermodynamics", "Optics", "Modern Physics"]:
            # 物理题目
            enhanced_obs["output_format_instruction"] = f"""{ocr_details}

    The OCR has extracted the following values from the diagram: {', '.join(all_texts)}

    Use these values to solve this {observation.get('subfield', 'physics')} problem.

    {original_instruction}

    ⚠️ REQUIRED: End with <answer>numerical_value</answer> or \\boxed{{numerical_value}}"""
        
        else:
            # 其他题目类型
            enhanced_obs["output_format_instruction"] = f"""{ocr_details}

    Using the text extracted from the image above, answer the question.

    {original_instruction}

    ⚠️ REMEMBER: You must provide your final answer using <answer>...</answer> or \\boxed{{...}}"""
        
        # 添加OCR结果到观察中，供模型参考
        enhanced_obs["ocr_performed"] = True
        enhanced_obs["ocr_results"] = {
            "all_texts": all_texts,
        ...
        # 如果检测到的文本很少，添加警告
        if num_detections == 0:
            enhanced_obs["ocr_warning"] = "⚠️ WARNING: No text detected by OCR. The image might not contain readable text."
        elif num_detections < 3:
            enhanced_obs["ocr_warning"] = f"⚠️ NOTE: Only {num_detections} text region(s) detected. Some text might be unclear."
        
        # 生成基于OCR结果的答案
        print(f"  - Generating answer based on OCR results")
        print(f"  - OCR detected texts: {all_texts[:5]}..." if len(all_texts) > 5 else f"  - OCR detected texts: {all_texts}")
        
        response, base_info = super().act(enhanced_obs)
        ...
        extra_info = base_info.copy()
        extra_info.update({
            "action_type": "final_answer_after_easyocr",
            "used_easyocr": True,
            "ocr_text_count": len(all_texts),
            "ocr_detections": num_detections,
            "stage": "post_ocr_analysis"
        })
        
        return response, extra_info
    
    def _find_relevant_text(self, texts: List[str], question: str) -> Optional[str]:
        """从OCR结果中找到与问题最相关的文本"""
        question_lower = question.lower()
        
        # 寻找包含问题关键词的文本
        keywords = re.findall(r'\b\w{4,}\b', question_lower)  # 提取4字母以上的词
