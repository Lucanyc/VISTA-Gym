...
class VLMAgentWithTools(VLMAgent):
    ...
    Secondary support: DeepEyes, GroundingDINO, DiagramFormalizer, EasyOCR, SAM2
    """
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.enable_sympy_geometry = tool_config.get("enable_sympy_geometry", False)  # 新增
        ...
        print(f"  - enable_sympy_geometry: {self.enable_sympy_geometry}")  # 新增
        ...
        self.sam2_tool = None  
        self.sympy_geometry_tool = None  # 新增
    ...
    def act(self, observation: Dict[str, Any]) -> Tuple[str, Dict[str, Any]]:
        ...
        #print(f"  - enable_tools: {self.enable_tools}")
        print(f"  - enable_chartmoe: {self.enable_chartmoe}")
        print(f"  - enable_sam2: {self.enable_sam2}")  # 新增
    ...
    def _get_available_tools(self, observation: Dict[str, Any]) -> List[int]:
        ...
        if self.enable_easyocr and self.easyocr_tool is not None:
            available.append(4)
        if self.enable_sam2 and self.sam2_tool is not None:  # 新增
            available.append(5)
        if self.enable_sympy_geometry and self.sympy_geometry_tool is not None:  # 新增
            available.append(6)
        ...
        # 对于几何任务的特殊处理
        if observation.get("is_geometry_task", False) and self.enable_sympy_geometry:  # 新增
