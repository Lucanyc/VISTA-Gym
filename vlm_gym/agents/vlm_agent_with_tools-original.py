...
class VLMAgentWithTools(VLMAgent):
    ...
    def act(self, observation: Dict[str, Any]) -> Tuple[str, Dict[str, Any]]:
        ...
        if observation.get("episode_start", False) or self._is_new_task(observation):
            ...
            print(f"  - Starting new task, reset tool history and context")
    ...
    def _ensure_answer_format(self, response: str, observation: Dict[str, Any]) -> str:
        ...
        # 检查是否已有 <think> 标签，避免嵌套
        if response.strip().startswith("<think>") and "</think>" in response:
            # 已有 think 标签，需要在其中添加 answer
            think_end = response.rfind("</think>")
            before_think_end = response[:think_end]
            
            # 尝试从响应中提取答案
            answer = self._extract_answer_from_content(response, observation)
            
            # 如果 think 内容中没有 answer 标签，添加它
            if "<answer>" not in before_think_end:
                return f"{before_think_end}\n<answer>{answer}</answer>\n</think>"
            else:
                return response
...
<answer>{answer}</answer>"""
    ...
    def _should_use_deepeyes(self, observation: Dict[str, Any]) -> bool:
        ...
        # ⭐ 新增：如果工具已启用且有图像，对于地图/视觉比较类问题应该使用
        # 这些类型的问题通常受益于视觉增强
        visual_analysis_patterns = [
            "which", "where", "locate", "find", "position",
        ...
        # 如果包含视觉分析相关词汇，应该考虑使用 DeepEyes
        if any(pattern in question for pattern in visual_analysis_patterns):
            print(f"  - Visual analysis task detected, DeepEyes may be helpful")
            return True
        
        # DeepEyes 原有的关键词检查
        deepeyes_keywords = [
            "zoom", "detail", "closer", "small", "tiny", 
            "hard to see", "unclear", "blurry",
            "rotate", "angle", "turn",
            "examine", "look closely", "magnify",
            "enhance", "clearer view"
        ]
        
        # 检查关键词
        if any(keyword in question for keyword in deepeyes_keywords):
            print(f"  - DeepEyes keywords detected in question")
            return True
...
<answer>{num_detections}</answer>"""
    ...
    def _build_deepeyes_prompt(self, observation: Dict[str, Any]) -> str:
        ...
        # 分析图像大小以智能确定缩放区域
        # 注意：实际大小需要从观察中获取，这里使用默认值
        image_width = 800  # 默认宽度
        image_height = 600  # 默认高度
